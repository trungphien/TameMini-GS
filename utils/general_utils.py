...
class DecayScheduler(object):
    ...
    def __call__(self, step):
        ...
        elif self.decay_name == 'logarithmic':
            if step > self.total_steps:
                return self.end
            # Công thức logarithmic decay: start - (start - end) * log(1 + step) / log(1 + total_steps)
            return self.end + (self.start - self.end) * (1 - np.log(1 + step) / np.log(1 + self.total_steps))
        else:
