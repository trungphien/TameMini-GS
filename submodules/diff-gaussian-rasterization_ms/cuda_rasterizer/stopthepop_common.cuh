...
template<uint32_t PATCH_WIDTH, uint32_t PATCH_HEIGHT>
// Nhận vào tile và Gaussian 2D, trả về giá trị phân phối Gaussian 2D tại vị trí có đóng góp lớn nhất trong tile
	...
	glm::vec2& max_pos)
{
	...
	{
		// Tính toán điểm góc của tile gần nhất với điểm trung tâm của Gaussian
		const float px = x_left * rect_min.x + (1.0f - x_left) * rect_max.x;
		const float py = y_above * rect_min.y + (1.0f - y_above) * rect_max.y;

		// Tính toán hướng di chuyển trên 2 cạnh xuất phát từ điểm góc
		// copysign trả về độ lớn của tham số thứ nhất và dấu của tham số thứ hai
		...
		// __saturatef giới hạn giá trị trong khoảng [0,1]
	...
	// Trả về giá trị phân phối Gaussian 2D tại vị trí max_pos
	return max_contrib_power;
}
...
__device__ inline int computeTilebasedCullingTileCount(const bool active, 
	...
	const uint2 rect_min_init, // Điểm góc trái-trên của vùng (bao gồm nhiều tile) cần xử lý
	const uint2 rect_max_init) // Điểm góc phải-dưới của vùng (bao gồm nhiều tile) cần xử lý
{	
	// Tính tổng số tile trong vùng cần xử lý.
	...
	if (active)
	{
		...
		for (int tile_idx = 0; tile_idx < tile_count_init && (!LOAD_BALANCING || tile_idx < SEQUENTIAL_TILE_THRESH); tile_idx++)
		{
			...
			// Tính toán tọa độ góc trái-trên và phải-dưới của tile, BLOCK_X và BLOCK_Y là kích thước của tile
			const glm::vec2 tile_min = {x * BLOCK_X, y * BLOCK_Y};
			const glm::vec2 tile_max = {(x + 1) * BLOCK_X - 1, (y + 1) * BLOCK_Y - 1};

			glm::vec2 max_pos;
			// Tính mức đóng góp tối đa của Gaussian trong tile
			...
			// Tăng tile_count nếu mức đóng góp nhỏ hơn ngưỡng
			tile_count += (max_opac_factor <= opacity_power_threshold_init);
		}
	}
	// Load balancing là kỹ thuật phân phối công việc đồng đều giữa các thread GPU để tối ưu hiệu năng
	if (!LOAD_BALANCING)
		return tile_count;
